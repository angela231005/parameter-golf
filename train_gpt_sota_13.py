...
class Hyperparameters:
    ...
    # 4-gram hash reuses same embedding table at near-zero extra param cost
    fourgram_enabled = bool(int(os.environ.get("FOURGRAM", "1")))
    ...
    # AR self-gen calibration sequences — 96 for better Hessian estimation
    gptq_ar_seqs = int(os.environ.get("GPTQ_AR_SEQS", 96))
    # GPTQ Hessian damping factor (relative to mean diagonal)
    ...
    ttt_chunk_size = int(os.environ.get("TTT_CHUNK_SIZE", 16384))  # smaller=more adaptations
    # --- Cautious Weight Decay (only apply WD where update would reduce |param|) ---
...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr: float, momentum: float, backend_steps: int,
                 nesterov: bool = True, weight_decay: float = 0.0, cautious_wd: bool = False):
        super().__init__(
            params,
            dict(lr=lr, momentum=momentum, backend_steps=backend_steps,
                 nesterov=nesterov, weight_decay=weight_decay, cautious_wd=cautious_wd),
        )
        self._built = False
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            cwd = group.get("cautious_wd", False)
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                if prev_ag_handle is not None:
                    ...
                    if wd > 0.0:
                        if cwd:
                            # Cautious WD: only apply where update opposes param (reduces |param|)
                            cwd_mask = (pp.data.float() * upd.to(pp.dtype).float() < 0).to(pp.dtype)
                            pp.data.mul_(1.0 - lr * wd * cwd_mask)
                        else:
                ...
                if sharded:
                    ...
                    if wd > 0.0:
                        if cwd:
                            # Cautious WD: only apply where update opposes param (reduces |param|)
                            cwd_mask = (p.data.float() * update.to(p.dtype).float() < 0).to(p.dtype)
                            p.data.mul_(1.0 - lr * wd * cwd_mask)
                        else:
...
class BigramHashEmbedding(nn.Module):
    def __init__(self, bigram_vocab_size: int, bigram_dim: int, model_dim: int, trigram: bool = False, fourgram: bool = False):
        super().__init__()
        self.bigram_vocab_size = bigram_vocab_size
        self._trigram = trigram
        self._fourgram = fourgram
    ...
    def fourgram_hash(self, tokens: Tensor) -> Tensor:
        """Hash (t-3, t-2, t-1, t) 4-grams into same embedding table. Zero extra params."""
        t = tokens.to(torch.int32)
        mod = self.bigram_vocab_size - 1
        out = torch.empty_like(t)
        out[..., :3] = mod
        out[..., 3:] = (36313 * t[..., 3:] ^ 27191 * t[..., 2:-1]
                        ^ 51497 * t[..., 1:-2] ^ 13337 * t[..., :-3]) % mod
        return out.long()
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bigram = BigramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim, trigram=bool(
            int(os.environ.get("TRIGRAM", "1"))), fourgram=bool(
            int(os.environ.get("FOURGRAM", "1")))) if bigram_vocab_size > 0 else None
...
def quantize_int6_gptq(weight, hessian=None, clip_range=31, block_size=128, damp_frac=0.005):
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            if H is not None:
                q, s = quantize_int6_gptq(
                    t, hessian=H, clip_range=cr, block_size=block_size, damp_frac=gptq_damp)
            else:
...
def run_legal_ttt(
    ...
    For each non-overlapping chunk of val_tokens:
      1. SCORE under torch.inference_mode() — no gradient tracking, no weight mutation possible.
      2. TRAIN on the already-scored chunk with SGD (score-first guarantee).
    The last chunk is scored but never trained on.

    Returns (val_loss, val_bpb) accumulated from the score-first pass.
    """
    ...
    # SGD on all params (full fine-tuning on the dequantized eval model)
    ttt_optimizer = torch.optim.SGD(
        [p for p in model.parameters() if p.requires_grad],
        lr=args.ttt_lr, momentum=0.9,
    )
