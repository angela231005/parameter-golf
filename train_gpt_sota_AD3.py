# ============================================================
# train_gpt_sota_AD3.py  —  Developed from strategy_AD2
# ============================================================
# Carries forward all AD2 features:
#   [AD2-1] RECUR_COUNT: repeat recur_layers N times (3 total passes by default)
#   [AD2-2] TTT AdamW: AdamW in test-time training, faster convergence per chunk
#   [AD2-3] Dynamic EMA: fast EMA early (explore), slow EMA late (stabilise)
#   [AD2-4] TTT LLRD + OneCycleLR schedule per chunk
#   [AD2-5] Adaptive recurrence ramp: +1 pass every recur_ramp_steps
#   [AD2-6] Per-pass untied adapters: tiny dim→adapter_dim→dim MLP per recurrence pass
#           Gated sigmoid, init zero → no effect at start, lets each pass specialise
...
class Hyperparameters():
    ...
    # [AD2] Dynamic EMA: ramp from ema_decay_early (fast/explore) → ema_decay → ema_decay_late (slow/stabilise)
    # Transitions: 0.0→0.3 = early phase, 0.7→1.0 = late phase
    ema_decay_early = float(os.environ.get('EMA_DECAY_EARLY', '0.990'))
    ema_decay_late = float(os.environ.get('EMA_DECAY_LATE', '0.9990'))
    ...
    # [AD2-6] Per-pass untied adapter: tiny dim→adapter_dim→dim MLP unique per recurrence pass
    # 0 = disabled; 32-64 recommended (adds <1% params, breaks weight-sharing symmetry)
    recur_adapter_dim = int(os.environ.get("RECUR_ADAPTER_DIM", "32"))

    # [AD2-7] Differential Attention: y = softmax(Q1,K1) - λ·softmax(Q2,K2)
    # Cancels attention noise, sharpens focus. Applied from diff_attn_start_layer onward.
    ...
    # [AD2] TTT optimizer: adamw (default, better) or sgd (original AD)
...
class RecurrencePassAdapter(nn.Module):
    """[AD2-6] Tiny gated bottleneck MLP unique per recurrence pass.
    Breaks weight-sharing symmetry: each pass can specialise.
    Adds <1% params. Init: gate=0 (no effect at start), last layer zeros.
    """
    def __init__(self, model_dim: int, adapter_dim: int, num_extra_passes: int):
        super().__init__()
        ...
        for adp in self.adps:
            nn.init.zeros_(adp[-1].weight)

    def apply_pass(self, x: Tensor, pass_idx: int) -> Tensor:
        """pass_idx: 1-based (first extra pass = 1)"""
        i = pass_idx - 1
        if i < 0 or i >= len(self.adps):
            return x
...
class GPT(nn.Module):
    def __init__(self, h: Hyperparameters):
        ...
        # [AD2-6] Per-pass adapters (one per extra pass)
        if h.recur_adapter_dim > 0 and h.recur_count > 0:
            self.recur_adapters = nn.ModuleDict({
                str(li): RecurrencePassAdapter(h.model_dim, h.recur_adapter_dim, h.recur_count)
                for li in self.recur_layers
            })
        else:
            self.recur_adapters = None
        # [AD2-7] Differential Attention: enable from diff_attn_start_layer onward
        if h.diff_attn_enabled:
            for i in range(h.diff_attn_start_layer, h.num_layers):
                self.blocks[i].attn.use_diff_attn = True
    ...
    def _get_virtual_layers(self) -> list[int]:
        ...
        virtual = []
        pass_nums = []  # [AD2-6] pass_num=0 for normal, 1..count for recurrence repeats
        inserted = False
        for i in range(n):
            virtual.append(i)
            pass_nums.append(0)
            if not inserted and i == self.recur_layers[-1]:
                ...
                for pass_idx in range(1, count + 1):
                    for rl in self.recur_layers:
                        virtual.append(rl)
                        pass_nums.append(pass_idx)
                inserted = True
        return virtual, pass_nums
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        virtual_layers, pass_nums = self._get_virtual_layers()
        ...
        for vi in range(num_enc):
            ...
            pass_num = pass_nums[vi]
            ve = self._get_ve(phys_idx, input_ids, ve_cache)
            x = self.blocks[phys_idx](x, x0, v_embed=ve)
            # [AD2-6] Per-pass adapter: specialise each recurrence repeat
            if pass_num > 0 and self.recur_adapters is not None:
                key = str(phys_idx)
                if key in self.recur_adapters:
                    x = self.recur_adapters[key].apply_pass(x, pass_num)
            skips.append(x)

        # Decoder phase with U-Net skip connections
        for vi in range(num_dec):
            phys_idx = virtual_layers[num_enc + vi]
            pass_num = pass_nums[num_enc + vi]
            ...
            if is_parallel_mode:
                ...
                ve = self._get_ve(phys_idx, input_ids, ve_cache)
                x = self.blocks[phys_idx](x, x0, v_embed=ve)
                # [AD2-6] Per-pass adapter: specialise each recurrence repeat (non-parallel path)
                if pass_num > 0 and self.recur_adapters is not None:
                    key = str(phys_idx)
                    if key in self.recur_adapters:
                        x = self.recur_adapters[key].apply_pass(x, pass_num)
...
def train_model(h: Hyperparameters, device: torch.device, val_data: ValidationData) -> None:
    ...
    if h.distributed:
        model = DDP(compiled_model, device_ids=[h.local_rank], broadcast_buffers=False,
                    find_unused_parameters=True)  # diff_attn uses diff_proj/proj selectively
    else:
    ...
    while True:
        ...
        with torch.no_grad():
            # [AD2] Dynamic EMA decay: fast early (explore) → slow late (stabilise)
            _ema_frac = training_frac(step, elapsed_ms)
            if _ema_frac < 0.3:
                _cur_ema_decay = h.ema_decay_early
            elif _ema_frac > 0.7:
                _cur_ema_decay = h.ema_decay_late
            else:
                _t = (_ema_frac - 0.3) / 0.4
                _cur_ema_decay = h.ema_decay_early + _t * (h.ema_decay_late - h.ema_decay_early)
            _cur_ema_decay = max(h.ema_decay_early, min(h.ema_decay_late, _cur_ema_decay))
            for name, t in base_model.state_dict().items():
                ema_state[name].mul_(_cur_ema_decay).add_(t.detach().float(), alpha=1.0 - _cur_ema_decay)
