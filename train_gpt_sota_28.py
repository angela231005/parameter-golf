...
class Hyperparameters:
    ...
    muon_wd = float(os.environ.get("MUON_WD", 0.090))   # Raki v6 Modification 3
    embed_wd = float(os.environ.get("EMBED_WD", 0.090))  # Raki v6: same as muon_wd for embeddings
    adam_wd = float(os.environ.get("ADAM_WD", 0.02))     # Raki v6: lower WD for scalar/control params
    qat_enabled = bool(int(os.environ.get("QAT_ENABLED", "0")))
    bigram_vocab_size = int(os.environ.get("BIGRAM_VOCAB_SIZE", 3072))
    # PR #1204: 112 saves budget for recurrence MLPs
    bigram_dim = int(os.environ.get("BIGRAM_DIM", 112))
    # OFF to match record — saves compute per step
    ...
    # Raki v6: sigmoid gates on U-Net skip connections (lerp blend vs additive)
    skip_gates_enabled = bool(int(os.environ.get("SKIP_GATES_ENABLED", "1")))
    # GPTQ calibration
    gptq_calib_batches = int(os.environ.get("GPTQ_CALIB_BATCHES", 256))
    gptq_block_size = int(os.environ.get("GPTQ_BLOCK_SIZE", 128))
    # AR self-gen calibration sequences — 32 is plenty for Hessian estimation
    gptq_ar_seqs = int(os.environ.get("GPTQ_AR_SEQS", 32))
    # --- Parallel Residuals (PR #1204 / modded-nanogpt PR #230) ---
    parallel_residual = bool(int(os.environ.get("PARALLEL_RESIDUAL", "1")))
    parallel_start_layer = int(os.environ.get(
        "PARALLEL_START_LAYER", 5))  # physical layer idx — L5+ = all decoder layers
    # --- Mini Depth Recurrence (PR #1204) ---
    # physical layers to repeat — L3,4,5 = one extra recurrence layer
    recur_layers = os.environ.get("RECUR_LAYERS", "3,4,5")
    recur_start_step = int(os.environ.get(
        "RECUR_START_STEP", 3000))  # delayed activation
    recur_count = int(os.environ.get("RECUR_COUNT", 1))  # times to repeat each recur layer (2=triple loop)
    repeat_untie_mlp = os.environ.get(
        "REPEAT_UNTIE_MLP", "full")  # 'full'|'down'|'none'
    # --- Legal Score-First TTT (PR #549: chunk=32768, lr=0.002 cosine, 3 epochs, SGD) ---
    ttt_enabled = bool(int(os.environ.get("TTT_ENABLED", "0")))
    ttt_lr = float(os.environ.get("TTT_LR", 0.002))         # PR #549 uses 0.002
    ttt_epochs = int(os.environ.get("TTT_EPOCHS", 3))
    ttt_chunk_size = int(os.environ.get("TTT_CHUNK_SIZE", 32768))  # PR #549: 32768 tokens/chunk
    ttt_freeze_blocks = int(os.environ.get("TTT_FREEZE_BLOCKS", 0))  # 0 = all blocks adapt
    ttt_optimizer_type = os.environ.get("TTT_OPTIMIZER", "sgd")  # 'sgd' or 'adamw' (PR #1440: adamw wins)
    # N-gram tilt: p_tilt = p_model * exp(beta * 1[t==bigram_hint]) / Z  (PR #1437)
    ...
    # Z-loss: penalise large log-partition-function to control logit magnitude (Gemma/PaLM)
    z_loss_weight = float(os.environ.get("Z_LOSS_WEIGHT", 1e-4))
    # Sequence-length curriculum: vary seq_len during training
    # SEQ_CURRICULUM="1500,3000" means: use shorter_lens[0] for steps<1500,
    # shorter_lens[1] for 1500<=step<3000, then train_seq_len from 3000+
    _seq_curriculum_str = os.environ.get("SEQ_CURRICULUM", "")
    _seq_curriculum_lens_str = os.environ.get("SEQ_CURRICULUM_LENS", "512,1024")
    seq_curriculum_thresholds: list = (
        [int(x) for x in _seq_curriculum_str.split(",") if x.strip()]
        if _seq_curriculum_str else []
    )
...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr: float, momentum: float, backend_steps: int,
                 ...
                 mousse: bool = False, mousse_beta: float = 0.95, mousse_eps: float = 1e-8):
        super().__init__(
            params,
            dict(lr=lr, momentum=momentum, backend_steps=backend_steps,
                 nesterov=nesterov, weight_decay=weight_decay,
                 mousse=mousse, mousse_beta=mousse_beta, mousse_eps=mousse_eps),
        )
        self._built = False
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                if prev_ag_handle is not None:
                    ...
                    pp.add_(upd.to(dtype=pp.dtype),
                            alpha=-lr * prev_m['scale'])
                ...
                # Mousse: diagonal Kronecker curvature preconditioning (arXiv:2603.09697)
                # G_prec[i,j] = G[i,j] / (sqrt(diag(G@G^T)[i]) * sqrt(diag(G^T@G)[j]))
                if group.get("mousse", False) and update.ndim == 2:
                    beta_m = group.get("mousse_beta", 0.95)
                    eps_m = group.get("mousse_eps", 1e-8)
                    L = (update.float() ** 2).sum(dim=1)  # [shard_rows] = diag(G@G^T)
                    R = (update.float() ** 2).sum(dim=0)  # [cols] = diag(G^T@G)
                    sp = self.state[m['p']]
                    if 'L_ema' not in sp:
                        sp['L_ema'] = L
                        sp['R_ema'] = R
                    else:
                        sp['L_ema'].mul_(beta_m).add_(L, alpha=1.0 - beta_m)
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,parallel_post_lambdas,parallel_resid_lambdas",
    ).split(",")
    if pattern
)
...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        ...
        clipped = torch.maximum(torch.minimum(
            t32, clip_abs[:, None]), -clip_abs[:, None])
        scale = (clip_abs / 127.0).clamp_min(1.0 / 127.0)
        q = torch.clamp(torch.round(
            clipped / scale[:, None]), -127, 127).to(torch.int8).contiguous()
        return q, scale.to(dtype=INT8_PER_ROW_SCALE_DTYPE).contiguous()
    clip_abs = float(torch.quantile(t32.abs().flatten(),
                     INT8_CLIP_Q).item()) if t32.numel() else 0.0
    scale = torch.tensor(clip_abs / 127.0 if clip_abs >
                         0 else 1.0, dtype=torch.float32)
    q = torch.clamp(torch.round(torch.clamp(
        t32, -clip_abs, clip_abs) / scale), -127, 127).to(torch.int8).contiguous()
    return q, scale
...
def dequantize_state_dict_int8(obj: dict[str, object]) -> dict[str, Tensor]:
    ...
    for name, q in obj["quantized"].items():
        ...
        if qmeta.get(name, {}).get("scheme") == "per_row" or s.ndim > 0:
            ...
            out[name] = (q.float() * s.view(q.shape[0], *
                         ([1] * (q.ndim - 1)))).to(dtype=dtype).contiguous()
        else:
            scale = float(s.item())
...
# --- Raki v6 coprime-stride multi-shard loader ---
# Replaces simple sequential TokenStream with diversity-increasing curriculum:
#   - mmap for zero-copy lazy loading
#   - coprime stride within each shard (full coverage, no repetition bias)
#   - adaptive shard weighting (alpha: 0.90→0.50) that broadens diversity over training
...
class DistributedTokenLoader:
    """Coprime-stride multi-shard loader (Raki v6). Samples windows across shards with
    increasing diversity over training, using coprime strides for full coverage."""
    ...
    def _init_pipeline(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> None:
        # Reset cursor states — block structure changes with seq_len
        self._cursor_phase[:] = 0
        self._cursor_block_count[:] = 0
        self._cursor_next[:] = 0
        self._cursor_start[:] = 0
        self._cursor_stride[:] = 1
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        if self._cfg is None or self._last_seq_len != seq_len:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            with torch.no_grad():
                ...
                w_q = (torch.clamp(torch.round(
                    w32 / scale[:, None]), -32, 31) * scale[:, None]).to(x.dtype)
            w = w + (w_q - w).detach()
...
class Rotary(nn.Module):
    def __init__(self, dim: int, base: float = 10000.0, train_seq_len: int = 1024, rope_dims: int = 0):
        ...
        inv_freq = 1.0 / (base ** (torch.arange(0, self.rope_dims,
                          2, dtype=torch.float32) / self.rope_dims))
...
class BigramHashEmbedding(nn.Module):
    def __init__(self, bigram_vocab_size: int, bigram_dim: int, model_dim: int, trigram: bool = False):
        ...
        self.proj = CastedLinear(
            bigram_dim, model_dim, bias=False) if bigram_dim != model_dim else None
...
class ValueEmbedding(nn.Module):
    ...
    def __init__(self, vocab_size: int, ve_dim: int, model_dim: int):
        ...
        self.proj = CastedLinear(
            ve_dim, model_dim, bias=False) if ve_dim != model_dim else None
        if self.proj is not None:
            nn.init.zeros_(self.proj.weight)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        attn_out, raw_v = self.attn(self.attn_norm(
            x_in) * self.ln_scale_factor, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
        x_out = x_in + \
    ...
    def forward_parallel(self, x_lane0: Tensor, x_lane1: Tensor, x0: Tensor,
                         q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor,
                         up_w: Tensor, down_w: Tensor,
                         ppl: Tensor, prl: Tensor,
                         v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor, Tensor | None]:
        """Parallel residual: attn reads lane0, MLP reads lane1, write to both lanes."""
        mix = self.resid_mix.to(dtype=x_lane0.dtype)
        # Attn reads lane0
        attn_in = mix[0][None, None, :] * x_lane0 + mix[1][None, None, :] * x0
        attn_out, raw_v = self.attn(self.attn_norm(attn_in) * self.ln_scale_factor,
                                    q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
        scaled_attn = self.attn_scale.to(dtype=attn_out.dtype)[
            None, None, :] * attn_out
        # MLP reads lane1
        ...
        new_lane0 = prl_d[0][None, None, :] * x_lane0 + ppl_d[0, 0][None,
                                                                    None, :] * scaled_attn + ppl_d[0, 1][None, None, :] * scaled_mlp
        new_lane1 = prl_d[1][None, None, :] * x_lane1 + ppl_d[1, 0][None,
                                                                    None, :] * scaled_attn + ppl_d[1, 1][None, None, :] * scaled_mlp
...
class GPT(nn.Module):
    def __init__(
        ...
        recur_layers: str = "",
        recur_count: int = 1,
        skip_gates_enabled: bool = False,
        z_loss_weight: float = 0.0,
    ):
        super().__init__()
        self._ve_target_dim = num_kv_heads * \
            (model_dim // num_heads)  # kv_dim for value projection
        if logit_softcap <= 0.0:
            raise ValueError(
                f"logit_softcap must be positive, got {logit_softcap}")
        ...
        self.recur_count = max(1, recur_count)
        # --- Parallel Residuals ---
        self.parallel_residual = parallel_residual
        self.parallel_start_layer = parallel_start_layer
        # --- Depth Recurrence ---
        self.recur_layer_indices = [
            int(x) for x in recur_layers.split(",") if x.strip()]
        self.recur_active = False  # activated at recur_start_step during training
        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        self.bigram = BigramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim, trigram=bool(
            int(os.environ.get("TRIGRAM", "1")))) if bigram_vocab_size > 0 else None
        ...
        self.skip_weights = nn.Parameter(torch.ones(
            self.num_skip_weights, model_dim, dtype=torch.float32))
        # Raki v6: sigmoid gates replace additive skip (lerp between skip and x)
        self.skip_gates = nn.Parameter(torch.zeros(
            self.num_skip_weights, model_dim, dtype=torch.float32)) if skip_gates_enabled else None
        ...
        self.qo_bank = nn.Parameter(torch.empty(
            2 * num_layers, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(
        ...
        self.ve_layer_indices = [int(x) for x in ve_layers.split(
            ",") if x.strip()] if ve_enabled else []
        ...
        # Parallel residual parameters (used for layers >= parallel_start_layer)
        if parallel_residual:
            # Cross-lane write coefficients: [num_layers, 2(to_lane), 2(from_sublayer:attn/mlp), D]
            self.parallel_post_lambdas = nn.Parameter(
                torch.ones(num_layers, 2, 2, model_dim, dtype=torch.float32)
            )
            # Per-lane residual scaling: [num_layers, 2(lanes), D]
            self.parallel_resid_lambdas = nn.Parameter(
                torch.full((num_layers, 2, model_dim),
                           math.sqrt(1.1), dtype=torch.float32)
            )
        self.final_norm = RMSNorm()
        self.lm_head = None if tie_embeddings else CastedLinear(
    ...
    def _get_ve(self, layer_idx: int, input_ids: Tensor, ve_cache: dict | None = None) -> Tensor | None:
        ...
        ve_base = ve_cache['ve'] if ve_cache is not None else self.ve_shared(
            input_ids)
        ve_idx = self.ve_layer_indices.index(layer_idx)
        return ve_base * self.ve_layer_scales[ve_idx].to(dtype=ve_base.dtype)

    def _run_backbone(self, x: Tensor, input_ids: Tensor) -> Tensor:
        """Shared backbone for forward() and forward_logits(): encoder → recurrence → decoder with parallel residuals."""
        n = self.num_layers
        x0 = x
        v0 = None
        skips: list[Tensor] = []
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                                      ...
                                      self.qo_bank[n +
                                                   i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                                      v_embed=ve, v0=v0)
        ...
        if self.recur_active:
            for _rc in range(self.recur_count):
                for pi in self.recur_layer_indices:
                    ve = self._get_ve(pi, input_ids, ve_cache)
                    x, _ = self.blocks[pi](x, x0,
                                           self.qo_bank[pi], self.kv_bank[pi], self.kv_bank[n + pi],
                                           self.qo_bank[n +
                                                        pi], self.mlp_up_bank[pi], self.mlp_down_bank[pi],
                                           v_embed=ve, v0=v0)
        # --- Decoder (with optional parallel residuals) ---
        x_lane0: Tensor | None = None
        x_lane1: Tensor | None = None
        ...
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                if parallel_active:
                    scaled_skip = self.skip_weights[i].to(dtype=x_lane0.dtype)[None, None, :] * skips.pop()
                    if self.skip_gates is not None:
                        g = torch.sigmoid(self.skip_gates[i].to(dtype=x_lane0.dtype))[None, None, :]
                        x_lane0 = torch.lerp(scaled_skip, x_lane0, g)
                        x_lane1 = torch.lerp(scaled_skip, x_lane1, g)
                    else:
                        x_lane0 = x_lane0 + scaled_skip
                        x_lane1 = x_lane1 + scaled_skip
                else:
            ...
            # Parallel residual path (for layers >= parallel_start_layer)
            if self.parallel_residual and bi >= self.parallel_start_layer:
                if not parallel_active:
                    x_lane0 = x
                    x_lane1 = x
                    parallel_active = True
                x_lane0, x_lane1, _ = self.blocks[bi].forward_parallel(
                    x_lane0, x_lane1, x0,
                    self.qo_bank[bi], self.kv_bank[bi], self.kv_bank[n + bi],
                    self.qo_bank[n +
                                 bi], self.mlp_up_bank[bi], self.mlp_down_bank[bi],
                    self.parallel_post_lambdas[bi], self.parallel_resid_lambdas[bi],
                    v_embed=ve, v0=v0)
            else:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.tie_embeddings:
            ...
            if self.lm_head is None:
                raise RuntimeError(
                    "lm_head is required when tie_embeddings=False")
            logits_proj = self.lm_head(x_flat)
        logits = self.logit_softcap * \
            torch.tanh(logits_proj / self.logit_softcap)
        logits_f = logits.float()
        main_loss = F.cross_entropy(logits_f, targets, reduction="mean")
        # Z-loss: penalise logit magnitude (Gemma/PaLM style; prevents entropy collapse)
        if self.training and self.z_loss_weight > 0.0:
            main_loss = main_loss + self.z_loss_weight * logits_f.logsumexp(-1).pow(2).mean()
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_logits = self.logit_softcap * \
                    torch.tanh(mtp_logits_proj / self.logit_softcap)
                mtp_loss_sum = mtp_loss_sum + \
                    F.cross_entropy(mtp_logits.float(),
                                    mtp_targets, reduction="mean")
                mtp_loss_count += 1
            if mtp_loss_count > 0:
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            for i, ws in enumerate(batch_ws):
                ...
                wlen = end - ws
                wlens.append(wlen)
                chunk = val_tokens[ws:end +
                                   1].to(dtype=torch.int64, device=device)
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        ...
        for pct in [0.9975, 0.9980, 0.9985, 0.9990, 0.9993, 0.9995, 0.9997, 0.9999, 0.99995, 0.99999, 1.0]:
            if pct < 1.0:
                row_clip = torch.quantile(t32.abs(), pct, dim=1)
            else:
                row_clip = t32.abs().amax(dim=1)
            s = (row_clip / clip_range).clamp_min(1.0 /
                                                  clip_range).to(torch.float16)
            q = torch.clamp(torch.round(
                t32 / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
    ...
    scale = torch.tensor(amax / clip_range if amax >
                         0 else 1.0, dtype=torch.float16)
    ...
    for pct in [0.9975, 0.9980, 0.9985, 0.9990, 0.9993, 0.9995, 0.9997, 0.9999, 0.99995, 0.99999, 1.0]:
        ...
        for i1 in range(0, cols, block_size):
            ...
            for i in range(count):
                w = W1[:, i]
                d = Hinv1[i, i]
                q = torch.clamp(torch.round(w / sf), -
                                clip_range, clip_range).to(torch.int8)
                Q1[:, i] = q
                err = (w - q.float() * sf) / d
...
def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    for i in range(n):
        ...
        if dk in sd:
            down_slices[i] = sd[dk]
            consumed.add(dk)
    out["qo_bank"] = torch.stack(qo_slices).to(
        dtype=template_sd["qo_bank"].dtype)
    out["kv_bank"] = torch.stack(kv_slices).to(
        dtype=template_sd["kv_bank"].dtype)
...
class _HessianBlock(nn.Module):
    ...
    def forward(self, x, x0, v_embed=None):
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(
            x_in) * self.ln_scale_factor, v_embed=v_embed)
        x_out = x_in + \
            self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(
...
class _HessianGPT(nn.Module):
    ...
    def forward(self, input_ids, target_ids):
        ...
        x_flat = x.reshape(-1, x.size(-1))
        targets = target_ids.reshape(-1)
        logits_proj = F.linear(
            x_flat, self.tok_emb.weight) if self.tie_embeddings else self.lm_head(x_flat)
        logits = self.logit_softcap * \
            torch.tanh(logits_proj / self.logit_softcap)
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            if H is not None:
                q, s = quantize_int6_gptq(
                    t, hessian=H, clip_range=cr, block_size=block_size)
            else:
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if s.ndim > 0:
            out[name] = (q.float() * s.float().view(q.shape[0],
                         *([1] * (q.ndim - 1)))).to(orig_dtype)
        else:
            out[name] = (q.float() * float(s.item())).to(orig_dtype)
    return out

# --- Legal Score-First TTT with N-gram Tilt + Eval-Time Hash Embedding ---
...
def run_legal_ttt(
    ...
    """Legal Score-First TTT with two eval-time enhancements:

    1. N-gram Tilt (from PR #1437):
       p_tilt = p_model * exp(beta * 1[t==bigram_hint]) / Z
       Bigram table built causally: each chunk's table is updated AFTER scoring.

    2. Eval-Time Hash Embedding (from PR #1460):
       Zero-init nn.Embedding(hash_emb_size, model_dim) created at eval start.
       hash = (prev_token * 2039 + curr_token) % hash_emb_size
       Residual added to tok_emb output via forward hook.
       Included in TTT SGD, adapts alongside model weights.

    Both are score-first: no update before scoring, fully causal.
    """
    vocab_size = args.vocab_size
    seq_len = args.eval_seq_len if args.eval_seq_len > 0 else args.train_seq_len
    chunk_seqs = max(1, args.ttt_chunk_size // seq_len)
    chunk_tokens = chunk_seqs * seq_len
    batch_seqs = max(1, min(chunk_seqs, 8))  # micro-batch for scoring & training

    # --- N-gram bigram count table (add-1 smoothed, causally updated) ---
    bg_counts = torch.ones(vocab_size, vocab_size, dtype=torch.float32, device=device)

    # --- Eval-Time Hash Embedding (from #1460) ---
    hash_emb: nn.Embedding | None = None
    hook_handle = None
    if args.hash_emb_size > 0:
        model_dim = args.model_dim
        hash_emb = nn.Embedding(args.hash_emb_size, model_dim)
        nn.init.zeros_(hash_emb.weight)
        hash_emb = hash_emb.to(device=device, dtype=torch.bfloat16)

        _hes = args.hash_emb_size  # capture in closure
        ...
        def _hash_hook(
            ...
            ids = inp[0]                           # [B, T]
            prev_ids = torch.zeros_like(ids)
            prev_ids[:, 1:] = ids[:, :-1]         # shift right; pos 0 gets token 0
            h = (prev_ids * 2039 + ids) % _hes    # [B, T]
            return out + hash_emb(h).to(out.dtype)

        hook_handle = model.tok_emb.register_forward_hook(_hash_hook)

    # --- Freeze first ttt_freeze_blocks blocks ---
    _frozen_block_ids = set(range(min(args.ttt_freeze_blocks, len(model.blocks))))
    for name, p in model.named_parameters():
        is_frozen = any(f"blocks.{bi}." in name for bi in _frozen_block_ids)
        p.requires_grad_(not is_frozen)

    # --- Optimiser: unfrozen model params + hash_emb at 10× LR ---
    _hash_ids: set[int] = set()
    if hash_emb is not None:
        _hash_ids = {id(p) for p in hash_emb.parameters()}
    _model_ttt_params = [p for p in model.parameters() if p.requires_grad]
    _hash_ttt_params: list = []
    if hash_emb is not None:
        _hash_ttt_params = list(hash_emb.parameters())
    _all_ttt_params = _model_ttt_params + _hash_ttt_params
    _pg: list[dict] = [{'params': _model_ttt_params, 'lr': args.ttt_lr, 'lr_scale': 1.0}]
    if _hash_ttt_params:
        _pg.append({'params': _hash_ttt_params, 'lr': args.ttt_lr * 10, 'lr_scale': 10.0})
    if getattr(args, 'ttt_optimizer_type', 'sgd') == 'adamw':
        # AdamW TTT (PR #1440: lr=0.01 AdamW > lr=0.002 SGD for depth-recurrent models)
        ttt_optimizer = torch.optim.AdamW(_pg, weight_decay=0.0)
    else:
        ttt_optimizer = torch.optim.SGD(_pg, momentum=0.9)
    total_tokens = val_tokens.numel() - 1
    chunk_starts = [i for i in range(0, total_tokens, chunk_tokens)
                    if total_tokens - i >= seq_len]

    # Split chunks across ranks
    my_l = (len(chunk_starts) * rank) // world_size
    my_r = (len(chunk_starts) * (rank + 1)) // world_size
    ...
    for ci, cs in enumerate(my_starts):
        ...
        if nseq == 0:
            continue
        xs = val_tokens[cs:cs + nseq * seq_len].reshape(nseq, seq_len).to(
            device=device, dtype=torch.int64)
        ys = val_tokens[cs + 1:cs + nseq * seq_len + 1].reshape(nseq, seq_len).to(
            device=device, dtype=torch.int64)
        ...
        with torch.inference_mode():
            for bi in range(0, nseq, batch_seqs):
                ...
                with torch.autocast("cuda", torch.bfloat16):
                    logits = model.forward_logits(bx)  # [B, T, vocab]
                lf = logits.float()

                # N-gram tilt: boost bigram-predicted next token by exp(beta)
                if args.ngram_beta > 0:
                    prev_toks = bx.reshape(-1)
                    hint_toks = bg_counts[prev_toks].argmax(dim=-1)
                    tilt = torch.zeros(B * T, vocab_size, device=device)
                    tilt.scatter_(1, hint_toks.unsqueeze(1), args.ngram_beta)
                    lf = lf + tilt.reshape(B, T, vocab_size)
        ...
        # Update bigram counts AFTER scoring (score-first, causal)
        if args.ngram_beta > 0:
            pf = xs.reshape(-1).long()
            nf = ys.reshape(-1).long()
            flat_idx = pf * vocab_size + nf
            bg_counts.reshape(-1).scatter_add_(
                0, flat_idx,
                torch.ones(len(flat_idx), dtype=torch.float32, device=device)
            )
        ...
        if ci < len(my_starts) - 1:
            ...
            if hash_emb is not None:
                hash_emb.train()
            # Global cosine LR decay across all chunks
            ...
            for _epoch in range(args.ttt_epochs):
                ...
                for bi in range(0, nseq, batch_seqs):
                    ...
                    with torch.autocast("cuda", torch.bfloat16):
                        tl = model(bx, by)
                    tl.backward()
                    torch.nn.utils.clip_grad_norm_(_all_ttt_params, 1.0)
                    ttt_optimizer.step()

        if rank == 0 and (ci + 1) % max(1, len(my_starts) // 10) == 0:
            elapsed = time.perf_counter() - t0
            print(f"ttt:chunk {ci+1}/{len(my_starts)} elapsed:{elapsed:.0f}s "
                  f"chunks/s:{(ci+1)/elapsed:.2f}")
    ...
    tpb = (token_count / byte_count).item()
...
def main() -> None:
    ...
    tok_params = [{"params": [base_model.tok_emb.weight],
    ...
    # Parallel residual parameters (small learned scalars)
    if base_model.parallel_residual:
    ...
    optimizer_tok = torch.optim.AdamW(
        ...
        eps=args.adam_eps,
        weight_decay=args.embed_wd,   # Raki v6: embed_wd separate from scalar adam_wd
        fused=True,
    )
    optimizer_muon = Muon(
        matrix_params,
        ...
        mousse=bool(int(os.environ.get("MOUSSE_ENABLED", "1"))),
        mousse_beta=float(os.environ.get("MOUSSE_BETA", "0.95")),
    )
    ...
    log0(
        f"parallel_residual:{args.parallel_residual} start_layer:{args.parallel_start_layer}")
    log0(
        f"depth_recurrence: layers={args.recur_layers} start_step={args.recur_start_step}")
    ...
    max_wallclock_ms = 1000.0 * \
        args.max_wallclock_seconds if args.max_wallclock_seconds > 0 else None
    ...
    if args.warmup_steps > 0:
        ...
        initial_optimizer_states = [copy.deepcopy(
    ...
    while True:
        ...
        if not base_model.recur_active and args.recur_layers and step >= args.recur_start_step:
            ...
            model = compiled_model
            log0(
                f"recurrence:activated step:{step} layers:{args.recur_layers}")
        zero_grad_all()
        # Sequence-length curriculum: shorter seqs early for better exploration
        if args.seq_curriculum_thresholds:
            _curr_seq_len = args.train_seq_len
            for _thresh, _slen in zip(args.seq_curriculum_thresholds, args.seq_curriculum_shorter_lens):
                if step < _thresh:
                    _curr_seq_len = _slen
                    break
        else:
        ...
        if args.swa_enabled and scale < 0.2 and step % args.swa_every == 0:
            if swa_state is None:
                swa_state = {name: t.detach().cpu().clone()
                             for name, t in base_model.state_dict().items()}
                swa_count = 1
        ...
        if args.lawa_enabled and step % args.lawa_freq == 0:
            lawa_queue.append({name: t.detach().cpu().clone()
                              for name, t in base_model.state_dict().items()})
        should_log_train = (
    ...
    ema_avg = {name: t.to(dtype=current_state[name].dtype)
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        current_state = base_model.state_dict()
        avg_state = {name: torch.zeros(
            t.shape, dtype=torch.float32, device='cpu') for name, t in current_state.items()}
        for snap in lawa_queue:
            for name in avg_state:
                avg_state[name] += snap[name].float()
        for name in avg_state:
            avg_state[name] /= len(lawa_queue)
            avg_state[name] = avg_state[name].to(
    ...
    if swa_state is not None and swa_count > 1:
        ...
        if swa_val_loss < ema_val_loss:
            log0(
                f"swa:selected (val_loss {swa_val_loss:.4f} < ema {ema_val_loss:.4f})")
        else:
            log0(
                f"ema:selected (val_loss {ema_val_loss:.4f} <= swa {swa_val_loss:.4f})")
            base_model.load_state_dict(ema_avg, strict=True)
    ...
    hessian_model.load_state_dict(
        {k: v.to(device) for k, v in unbanked_sd.items()
         if k in hessian_model.state_dict()},
        strict=False,
    )
    # Autoregressive self-generated calibration (no external data)
    log0(
        f"gptq:generating autoregressive calibration data ({args.gptq_ar_seqs} seqs x {args.train_seq_len} tokens, temp=0.8)...")
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd, {"mlp", "attn"}, hessians=hessians, block_size=args.gptq_block_size)
    ...
    for name, info in quant_meta.items():
        ...
        if s.ndim > 0:
            ones_mask = (q.abs() == 1)
            if ones_mask.any():
                row_idx = torch.arange(q.shape[0]).unsqueeze(
                    1).expand_as(q)[ones_mask]
    ...
    if master_process:
        ...
        log0(
            f"Total submission size int6+{comp_name}: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    ...
    # eval_model needs recurrence active (it represents the final model state)
    ...
    # --- Legal Score-First TTT (PR #549: +~0.0025 BPB gain) ---
    if args.ttt_enabled:
        log0(
            f"ttt:starting lr:{args.ttt_lr} epochs:{args.ttt_epochs} "
            f"chunk_size:{args.ttt_chunk_size} freeze_blocks:{args.ttt_freeze_blocks} "
            f"ngram_beta:{args.ngram_beta} hash_emb_size:{args.hash_emb_size} "
            f"(Legal Score-First, PR #461 protocol)")
        torch.cuda.synchronize()
        ...
        log0(f"ttt:score_first val_loss:{ttt_val_loss:.4f} val_bpb:{ttt_val_bpb:.4f} "
             f"ttt_time:{ttt_elapsed:.0f}ms")
        log0(f"ttt:score_first_exact val_loss:{ttt_val_loss:.8f} val_bpb:{ttt_val_bpb:.8f}")
        # Post-TTT sliding window eval (model is now adapted)
        if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
            torch.cuda.synchronize()
            t_ttt_slide = time.perf_counter()
            ttt_sw_loss, ttt_sw_bpb = eval_val_sliding(
                args, eval_model, rank, world_size, device,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
