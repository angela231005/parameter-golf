...
try:
    ...
    try:
        ...
        def flash_attn_3_func(q, k, v, causal=True):
            ...
            B, T_q, Hq, D = q.shape
            _, T_k, Hkv, _ = k.shape
            q = q.transpose(1, 2).contiguous()  # (B, Hq, T, D)
            k = k.transpose(1, 2).contiguous()  # (B, Hkv, T, D)
            v = v.transpose(1, 2).contiguous()  # (B, Hkv, T, D)
            # Handle GQA: k/v may have fewer heads than q (Grouped Query Attention)
            if Hkv != Hq:
                groups = Hq // Hkv
...
class Hyperparameters():
    ...
    prequant_ttt_llrd = float(os.environ.get("PREQUANT_TTT_LLRD", 0.9))
    prequant_ttt_schedule = os.environ.get("PREQUANT_TTT_SCHEDULE", "onecycle")
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        if getattr(self, 'context_memory_active', False) and hasattr(self, 'context_values'):
            if not hasattr(self, '_context_proj_weight'):
                ...
                self._context_proj_weight = torch.randn((self.context_values.size(-1), x.size(-1)), generator=gen, device=x.device, dtype=x.dtype)
...
def prequant_ttt_adapt_adamw(
    ...
    base_lr = h.prequant_ttt_lr
    llrd = getattr(h, 'prequant_ttt_llrd', 1.0)
    ...
    for name, p in base_model.named_parameters():
        ...
        if "blocks." in name:
            try:
                ...
                depth = num_blocks - 1 - layer_idx
                layer_scale = (llrd ** depth)
            except:
                pass
        elif "tok_emb" in name or "ve_" in name:
            layer_scale = (llrd ** num_blocks)
            
        param_groups.append({"params": [p], "lr": base_lr * layer_scale, "weight_decay": 0.0})
    ...
    scheduler = None
    schedule_type = getattr(h, 'prequant_ttt_schedule', "cosine").lower()
    
    if schedule_type == "onecycle" and h.prequant_ttt_epochs > 0:
        max_lrs = [pg["lr"] for pg in param_groups]
        scheduler = torch.optim.lr_scheduler.OneCycleLR(
            optimizer, max_lr=max_lrs, total_steps=h.prequant_ttt_epochs, 
            pct_start=0.1, anneal_strategy='cos', div_factor=10.0, final_div_factor=100.0
        )
    elif h.prequant_ttt_cosine_decay and h.prequant_ttt_epochs > 0:
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=h.prequant_ttt_epochs, eta_min=base_lr * 0.1)
