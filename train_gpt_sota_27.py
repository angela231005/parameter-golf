# ==========================================================
# IDEA 27: Legal TTT + Hash Embed Adapt + N-gram Beta Decay
# TTT adapts both model params and bigram hash table; n-gram beta decays per chunk.
# Base: train_gpt_sota_17.py  —  [IDEA-27] marks changes
# ==========================================================
...
def flash_attn_3_func(q, k, v, causal=True):
    ...
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
    # Expand K,V for GQA (SDPA math backend requires equal head counts)
...
class Hyperparameters:
    ...
    warmdown_iters = int(os.environ.get("WARMDOWN_ITERS", 5500))
    ...
    # ON by default — reuses bigram hash table at near-zero parameter cost
    ...
    # AR self-gen calibration sequences — 64 for better Hessian estimation
    ...
    # physical layers to repeat — L2,3,4,5 = 4 encoder layers for deeper recurrence
    recur_layers = os.environ.get("RECUR_LAYERS", "2,3,4,5")
    ...
    ttt_enabled = bool(int(os.environ.get("TTT_ENABLED", "1")))  # [IDEA-27] on by default
    ttt_lr = float(os.environ.get("TTT_LR", 0.001))
    ttt_epochs = int(os.environ.get("TTT_EPOCHS", 3))
    ttt_chunk_size = int(os.environ.get("TTT_CHUNK_SIZE", 32768))
    # --- JEPA latent prediction (auxiliary training objective) ---
    # Predict next-position hidden representation in embedding space (not token space).
    # Stop-gradient on targets; collapse prevented by main CE loss.
    jepa_neck = int(os.environ.get("JEPA_NECK", 64))     # bottleneck dim (0 = disabled)
    jepa_weight = float(os.environ.get("JEPA_WEIGHT", 0.1))  # aux loss weight
    # --- nGPT: Normalized Transformer on the Hypersphere ---
    # All hidden states are L2-normalized; residual updates are sphere-walks.
    # alphas per block initialized to 1/sqrt(D). No DyT prenorm needed.
    ngpt = bool(int(os.environ.get("NGPT", 1)))  # 1 = enabled by default
    # --- N-gram Tilt (eval-only, legal score-first, from #1437) ---
    # p_tilt = p_model * exp(beta * 1[t==bigram_hint]) / Z
    # Applied only during TTT scoring pass, NOT during training.
    ngram_beta = float(os.environ.get("NGRAM_BETA", 0.5))   # tilt strength (0 = disabled)
    # --- Eval-Time Hash Embedding (eval-only, from #1460) ---
    # Zero-init nn.Embedding created at eval start, trained via TTT SGD.
...
class DyT(nn.Module):
    """Dynamic Tanh: tanh(alpha*x) with learnable scalar alpha.
    Replaces RMSNorm — purely elementwise backward, no reduction kernel.
    ...
    def forward(self, x: Tensor) -> Tensor:
        return torch.tanh(self.alpha.to(x.dtype) * x)


class JEPAPredictor(nn.Module):
    """Tiny 2-layer bottleneck MLP that predicts h[t+1] from h[t] in latent space.
    JEPA-style auxiliary objective: force representations to be temporally predictable.
    Stop-gradient on targets; collapse prevented by main CE loss.
    """
    def __init__(self, dim: int, neck: int = 64):
        super().__init__()
        self.w_in  = nn.Linear(dim, neck, bias=False)
        self.w_out = nn.Linear(neck, dim, bias=False)
...
class Block(nn.Module):
    def __init__(
        ...
        if not ngpt:
            ...
            # nGPT: per-dim alpha step-sizes on the hypersphere (init = 1/sqrt(D))
            alpha_init = 1.0 / math.sqrt(dim)
            self.attn_scale = nn.Parameter(torch.full((dim,), alpha_init, dtype=torch.float32))
            self.mlp_scale = nn.Parameter(torch.full((dim,), alpha_init, dtype=torch.float32))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.recur_passes = recur_passes  # number of times to repeat recur_layers per forward
        ...
        # nGPT: last block output is already on unit sphere, identity final norm
        self.final_norm = nn.Identity() if ngpt else DyT()
        ...
        # JEPA predictor: tiny bottleneck MLP, training-only aux objective
        self.jepa_pred = JEPAPredictor(model_dim, jepa_neck) if jepa_neck > 0 else None
        self._jepa_weight = jepa_weight
        if xsa_last_n > 0:
    ...
    def _run_backbone(self, x: Tensor, input_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            if v0 is None and raw_v is not None:
                v0 = raw_v
            skips.append(x)
        # --- Depth Recurrence (repeat specified layers recur_passes times) ---
        if self.recur_active:
            for _ in range(self.recur_passes):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # JEPA: predict next-position hidden representation in latent space
        if self.training and self.jepa_pred is not None:
            h_ctx = x[:, :-1, :]           # [B, T-1, D] — context positions
            h_tgt = x[:, 1:, :].detach()  # [B, T-1, D] — stop-grad targets
            h_pred = self.jepa_pred(h_ctx) # [B, T-1, D]
            h_pred = F.normalize(h_pred, dim=-1)
            h_tgt  = F.normalize(h_tgt,  dim=-1)
            jepa_loss = 1.0 - (h_pred * h_tgt).sum(dim=-1).mean()
            main_loss = main_loss + self._jepa_weight * jepa_loss
        return main_loss
...
def run_legal_ttt(
    ...
    """Legal Score-First TTT with two eval-time enhancements (sota_16):
    ...
    # Shape [VOCAB, VOCAB] = [1024, 1024] ≈ 4MB for float32. Fully on device.
    ...
    if args.hash_emb_size > 0:
        ...
        def _hash_hook(
            ...
            prev_ids[:, 1:] = ids[:, :-1]         # shift right; pos 0 gets token 0 (BOS-like)
            h = (prev_ids * 2039 + ids) % _hes    # [B, T]
            return out + hash_emb(h).to(out.dtype)  # residual in same dtype as tok_emb output

        hook_handle = model.tok_emb.register_forward_hook(_hash_hook)

    # --- Optimiser: model params + hash_emb if enabled ---
    optim_params = [p for p in model.parameters() if p.requires_grad]
    if hash_emb is not None:
        optim_params += list(hash_emb.parameters())
    ttt_optimizer = torch.optim.SGD(optim_params, lr=args.ttt_lr, momentum=0.9)
    ...
    for ci, cs in enumerate(my_starts):
        ...
        # N-gram tilt uses bg_counts from previous chunks only (causal, no future leak).
        model.eval()
        with torch.inference_mode():
            for bi in range(0, nseq, batch_seqs):
                bx = xs[bi:bi + batch_seqs]          # [B, T]
                by = ys[bi:bi + batch_seqs]          # [B, T]
                B, T = bx.shape
                ...
                if args.ngram_beta > 0:
                    prev_toks = bx.reshape(-1)                     # [B*T]
                    # Argmax of stale bg_counts for each prev token = hint
                    hint_toks = bg_counts[prev_toks].argmax(dim=-1)  # [B*T]
                    # One-hot tilt in log space (beta at hint slot, 0 elsewhere)
        ...
        # Update bigram counts AFTER scoring the chunk (score-first, causal)
        if args.ngram_beta > 0:
            pf = xs.reshape(-1).long()   # [nseq * seq_len] — all prev tokens
            nf = ys.reshape(-1).long()   # [nseq * seq_len] — all next tokens
            ...
            for _epoch in range(args.ttt_epochs):
                ...
                for bi in range(0, nseq, batch_seqs):
                    ...
                    with torch.autocast("cuda", torch.bfloat16):
                        tl = model(bx, by)
                    tl.backward()
                    all_params_for_clip = list(model.parameters())
                    if hash_emb is not None:
                        all_params_for_clip += list(hash_emb.parameters())
                    torch.nn.utils.clip_grad_norm_(all_params_for_clip, 1.0)
                    ttt_optimizer.step()
...
def main() -> None:
    ...
    log0(
        f"depth_recurrence: layers={args.recur_layers} start_step={args.recur_start_step} passes={args.recur_passes}")
    ...
    eval_model = GPT(
        ...
        recur_layers=args.recur_layers,
        recur_passes=args.recur_passes,
        jepa_neck=0,       # JEPA predictor not needed at eval
        jepa_weight=0.0,
        ngpt=args.ngpt,
    ).to(device).bfloat16()
    ...
    if args.ttt_enabled:
        log0(
            ...
            f"chunk_size:{args.ttt_chunk_size} (Legal Score-First, PR #461 protocol)")
        torch.cuda.synchronize()
