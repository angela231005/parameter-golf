...
# Auto-install required packages when running in Kaggle/Colab.
...
# Use flash_attn library when available (native GQA, no K/V expansion, fullgraph-compile safe).
# Falls back to PyTorch SDPA automatically if flash_attn is not installed.
try:
...
class Hyperparameters:
    ...
    # QAT fires when LR scale drops below this fraction (0.10 = last ~10% of warmdown).
    # Lower value = later QAT = more stable training before quantization kicks in.
    late_qat_threshold = float(os.environ.get("LATE_QAT_THRESHOLD", 0.10))
    # Step-based QAT trigger: 0 = disabled (use late_qat_threshold only).
    # Disabled by default so time-based training isn't affected by a hard step count.
    ...
    # [22AD] Mod D: Pre-quant AdamW TTT (strategy_AD / PR #1423)
    # NOTE: DISABLED by default — uses val_tokens before eval, violates rules.
    ...
    # [22AD] EMA decay configurable (was hardcoded 0.997)
    ema_decay_hp = float(os.environ.get("EMA_DECAY", 0.997))
    ...
    ttt_chunk_size = int(os.environ.get("TTT_CHUNK_SIZE", 65536))  # 65536: 50% fewer optimizer steps, same FLOPs
...
class GPT(nn.Module):
    ...
    def _run_backbone(self, x: Tensor, input_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                if parallel_active:
                    skip_add = self.skip_weights[i].to(dtype=x_lane0.dtype)[
                        None, None, :] * skips.pop()
                    x_lane0 = x_lane0 + skip_add
                    x_lane1 = x_lane1 + skip_add
                else:
                    x = x + \
                        self.skip_weights[i].to(dtype=x.dtype)[
                            None, None, :] * skips.pop()
...
# --- [22AD] Mod D: Pre-quant AdamW TTT (ported from strategy_AD / PR #1423) ---
...
def prequant_ttt_adapt_adamw(
    ...
    """AdamW TTT: fine-tune on val data BEFORE GPTQ (baked into the artifact)."""
    seq_len = args.train_seq_len
    total_seqs = (val_tokens.numel() - 1) // seq_len
    batch_seqs = args.prequant_ttt_batch_seqs
    if args.prequant_ttt_freeze_blocks > 0:
        for i, block in enumerate(base_model.blocks):
            if i < args.prequant_ttt_freeze_blocks:
                for p in block.parameters():
                    p.requires_grad_(False)
    ttt_params = [p for p in base_model.parameters() if p.requires_grad]
    log0(f"prequant_ttt:trainable={sum(p.numel() for p in ttt_params)} "
         f"frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)}")
    optimizer = torch.optim.AdamW(ttt_params, lr=args.prequant_ttt_lr, weight_decay=0.0)
    scheduler = None
    if args.prequant_ttt_cosine_decay:
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=args.prequant_ttt_epochs, eta_min=args.prequant_ttt_lr * 0.1)
    my_start = (total_seqs * rank) // world_size
    my_end = (total_seqs * (rank + 1)) // world_size
    base_model.train()
    t0 = time.perf_counter()
    for epoch in range(args.prequant_ttt_epochs):
        ...
        for bs in range(my_start, my_end, batch_seqs):
            ...
            torch.nn.utils.clip_grad_norm_(ttt_params, args.prequant_ttt_grad_clip)
            optimizer.step()
        ...
        if scheduler is not None:
            scheduler.step()
        log0(f"prequant_ttt:epoch {epoch+1}/{args.prequant_ttt_epochs} "
             f"loss:{epoch_avg:.4f} time:{time.perf_counter() - t0:.1f}s")
    for p in base_model.parameters():
        p.requires_grad_(True)
    base_model.eval()
    log0(f"prequant_ttt:done elapsed={time.perf_counter() - t0:.1f}s")
...
def main() -> None:
    ...
    ema_decay = args.ema_decay_hp  # [22AD] configurable via EMA_DECAY env var
    ...
    if swa_state is not None and swa_count > 1:
        ...
        if swa_val_loss < ema_val_loss:
            ...
            base_model.load_state_dict(ema_avg, strict=True)
    # [22AD] Mod D: Pre-quant AdamW TTT — fine-tune on val data BEFORE GPTQ
    if args.prequant_ttt_enabled:
        log0("prequant_ttt:starting AdamW fine-tune before GPTQ...")
